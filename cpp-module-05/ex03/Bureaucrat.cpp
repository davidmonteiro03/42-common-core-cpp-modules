...
/*   Updated: 2025/04/03 17:18:46 by dcaetano         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void Bureaucrat::executeForm(const AForm &f) const
{
	...
	{
		f.execute(*this);
		std::cout << this->__name << " executed " << f.getName() << std::endl;
	}
	catch (const std::exception &e)
	{
		std::cerr << this->__name << " couldn't execute " << f.getName() << " because " << e.what() << "." << std::endl;
	}
}
