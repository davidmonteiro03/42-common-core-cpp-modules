...
/*   Updated: 2025/02/15 21:46:27 by dcaetano         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	this->index = 0;
	std::cout << "Welcome. There was created an empty phonebook for up to 8 contacts." << std::endl;
}

PhoneBook::~PhoneBook(void) { std::cout << "All contacts from phonebook were removed. See you next time." << std::endl; }
...
void PhoneBook::add_contact(void)
{
	...
	while (!std::cin.eof() && str == "")
	{
		...
		if (std::getline(std::cin, str) && str != "")
		{
			...
			std::cout << std::endl
					  << "This contact was added to phonebook."
					  << std::endl;
		}
	}
	this->index++;
...
{
	std::cout << std::endl
			  << "Information of this contact:" << std::endl;
	if (!contact.get_first_name().size())
	{
		std::cout << std::endl
				  << "Error searching contact..." << std::endl;
		return;
	}
	std::cout << std::endl
...
Contact PhoneBook::get_contact(int index) { return this->contacts[index % 8]; }
